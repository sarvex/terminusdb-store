...
lazy_static! {
    ...
    static ref EXECUTOR: TaskExecutor = RUNTIME.executor();
}

fn task_sync<T:'static+Send,E:'static+Send,F:'static+Future<Item=T,Error=E>+Send>(future: F) -> Result<T,E> {
    let (tx, rx) = oneshot::channel();
    EXECUTOR.spawn(future.then(move |r| tx.send(r)).map(|_|()).map_err(|_|()));
